...
# Map GHL columns for dashboard logic
if not opps.empty:
    mapping = {'value': 'Opportunity Value', 'owner': 'Opportunity Owner', 'pipeline': 'Pipeline', 'stage': 'Stage', 'status': 'Status', 'source': 'Lead Source'}
    opps.rename(columns={k:v for k,v in mapping.items() if k in opps.columns}, inplace=True)
    if 'created_date' in opps.columns:
        opps['Created Date'] = pd.to_datetime(opps['created_date']).dt.date
...
with tabs[5]:
    if not contacts.empty:
        ...
        if 'first_attribution' in contacts.columns and 'latest_attribution' in contacts.columns:
            f_counts = contacts['first_attribution'].value_counts().reset_index(name='First')
            l_counts = contacts['latest_attribution'].value_counts().reset_index(name='Latest')
            attr_df = pd.merge(f_counts, l_counts, on='index', how='outer').fillna(0).head(15)
            attr_df.rename(columns={'index': 'Source'}, inplace=True)
